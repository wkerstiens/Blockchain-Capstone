...
contract SolnSquareVerifier is ERC721Mintable {
    ...
    mapping(uint256 => Solutions) private uniqueSolutions;

    // TODO Create an event to emit when a solution is added
    event SolutionAdded(address _address, uint256 index);

    // TODO Create a function to add the solutions to the array and emit the event
    function _addSolution(address _address, uint256 _index, bytes32 key) internal {
    ...
    function mintToken( address _address, uint256 tokenId, uint[2] memory a, uint[2] memory b, unit[2] memory c, uint[2] memory input) public {
        bytes32 key = keccak256(abi.encodedPacked(a, b, c, input));
